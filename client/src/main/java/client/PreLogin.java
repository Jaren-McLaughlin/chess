...
public class PreLogin implements CommandHandler {
    public String executeCommand(ClientSession clientSession, ServerFacade serverFacade, String input) {
        ...
        return switch (command) {
            case "help" -> help();
            case "login" -> login(clientSession, serverFacade, parameters);
            case "quit" -> quit();
            case "register" -> register(clientSession, serverFacade, parameters);
    ...
    private String login(ClientSession clientSession, ServerFacade serverFacade, String[] input) {
        if (input.length < 2) {
        ...
        clientSession.setAuthToken(response.authToken());
    ...
    private String register(ClientSession clientSession, ServerFacade serverFacade, String[] input) {
        if (input.length < 3) {
